...
import custom_components.powersensor.powersensor_discovery_service as mod
...
async def test_second_remove_service_skipped_when_already_pending(
    ...
    for cancel in service._pending_removals.values():
        cancel()
...
@pytest.mark.asyncio
async def test_discovery_service_start_calls_service_browser(
    hass: HomeAssistant,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that start() instantiates a ServiceBrowser using the HA zeroconf instance.

    Lines 49-51 of powersensor_discovery_service.py: the real ServiceBrowser
    and homeassistant.components.zeroconf imports are exercised here.
    """
    import custom_components.powersensor.powersensor_discovery_service as mod

    mock_browser = Mock()
    monkeypatch.setattr(mod, "ServiceBrowser", lambda *a, **kw: mock_browser)
